...
if [ "$OS" = "Darwin" ]; then
  # macOS
  sed -i '' 's/aarch64/x86_64/g' .config
  sed -i '' 's/"rust-analyzer.cargo.target": "aarch64-unknown-linux-gnu"/"rust-analyzer.cargo.target": "x86_64-unknown-linux-gnu"/g' kernel.code-workspace
else
  # Linux and others
